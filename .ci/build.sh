...
then
	...
	cmake -G Ninja -D SDL_SHARED=ON -D SDL_STATIC=OFF \
		...
		\
		-D SDL_VIDEO=ON -D SDL_X11=OFF -D SDL_X11_SHARED=OFF -D SDL_WAYLAND=OFF -D SDL_WAYLAND_SHARED=OFF \
		-D SDL_WAYLAND_LIBDECOR=OFF -D SDL_WAYLAND_LIBDECOR_SHARED=OFF -D SDL_WAYLAND_QT_TOUCH=OFF -D SDL_RPI=OFF -D SDL_VIVANTE=OFF \
		-D SDL_VULKAN=OFF -D SDL_KMSDRM=ON -D SDL_KMSDRM_SHARED=ON -D SDL_OFFSCREEN=OFF -D SDL_RENDER=ON -D SDL_GPU=OFF \
		-D SDL_DIALOG=OFF -D SDL_OPENGL=ON -D SDL_OPENGLES=ON \
		\
		...
		-D SDL_LOADSO=ON -D SDL_CPUINFO=ON -D SDL_FILESYSTEM=ON -D SDL_DLOPEN=OFF -D SDL_SENSOR=OFF -D SDL_LOCALE=OFF \
		\
		-D SDL_CAMERA=OFF \
		\
		-D "CMAKE_TOOLCHAIN_FILE=$toolchain_file" -D "CMAKE_INSTALL_PREFIX=$cwd_root/archive_tmp/usr" \
