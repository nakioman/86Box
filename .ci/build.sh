...
EOF
	...
	then
		...
		"$gpiod_meson_venv/bin/meson" setup "$gpiod_root/build" "$gpiod_root" --prefix="$gpiod_install" -Dtools=disabled -Dtests=disabled -Ddbus=disabled -Dsystemd=disabled -Dbindings-glib=disabled || exit 99
...
then
	...
	if [ ! -d "$prefix" ]
	then
		rm -rf "$cache_dir/SDL2-"* "$cache_dir/SDL3-"* # remove old versions
	...
	cmake -G Ninja -D SDL_SHARED=ON -D SDL_STATIC=OFF \
		...
		-D SDL_DIALOG=OFF -D SDL_OPENGL=OFF -D SDL_OPENGLES=OFF -D SDL_UNIX_CONSOLE_BUILD=ON -D SDL_TEST_LIBRARY=OFF -D SDL_TESTS=OFF \
		\
