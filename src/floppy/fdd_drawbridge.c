    ...
    for (uint32_t bit = 0; bit < dataLengthInBits; bit++) {
        ...
        if (decoded == MFM_SYNC_SECTOR_HEADER) {
            ...
            if (sector.header.cylinder != cylinder) {
                // TEMPORARILY DISABLED: cylinder mismatch errors for testing
                // sector.headerErrors++;
...
/* Debug helper to examine raw track data */
static void 
dump_track_data_sample(uint8_t *data, int len, const char *label)
{
    drawbridge_fdd_log("DrawBridge: %s - dumping first 64 bytes:\n", label);
    for (int i = 0; i < 64 && i < len; i += 16) {
        drawbridge_fdd_log("  %04X: ", i);
        for (int j = 0; j < 16 && i + j < len && i + j < 64; j++) {
            drawbridge_fdd_log("%02X ", data[i + j]);
        }
        drawbridge_fdd_log("\n");
    }
    
    /* Look for potential sync patterns */
    drawbridge_fdd_log("DrawBridge: Looking for MFM patterns in first 1024 bytes:\n");
    int sync_4489_count = 0, a1_count = 0, fe_count = 0, fb_count = 0;
    
    for (int i = 0; i < 1024 && i < len - 1; i++) {
        /* Look for 0xA1 bytes */
        if (data[i] == 0xA1) a1_count++;
        
        /* Look for 0x4489 words (A1 sync in MFM) */
        if (i < len - 1) {
            uint16_t word = (data[i] << 8) | data[i + 1];
            if (word == 0x4489) sync_4489_count++;
        }
        
        /* Look for address marks */
        if (data[i] == 0xFE) fe_count++;
        if (data[i] == 0xFB) fb_count++;
    }
    
    drawbridge_fdd_log("  Found patterns: A1=%d, 4489=%d, FE=%d, FB=%d\n", 
                      a1_count, sync_4489_count, fe_count, fb_count);
}
...
read_sector_from_device(int drive, int track, int head, int sector, uint8_t *buffer)
{
    ...
    /* Calculate the actual cylinder for DrawBridge hardware */
    /* In 86Box, track parameter is logical track = cylinder * heads + head */
    /* For standard floppies, there are 2 heads, so cylinder = track / 2 */
    uint32_t drawbridge_cylinder = track;
    
    /* Seek to the desired track if not already there */
    if (dev->track != track) {
        response = arduino_interface_select_track(dev->arduino, drawbridge_cylinder);
        if (response != DIAGNOSTIC_RESPONSE_OK) {
            drawbridge_fdd_log("DrawBridge: Failed to seek to cylinder %d: %d", drawbridge_cylinder, response);
            if (response == DIAGNOSTIC_RESPONSE_MEDIA_TYPE_MISMATCH) {
                drawbridge_fdd_log(" (MEDIA_TYPE_MISMATCH - disk geometry issue)");
            }
            drawbridge_fdd_log("\n");
            memset(buffer, 0, 512);
            return;
    ...
    response = arduino_interface_select_surface(dev->arduino, 
        (head == 0) ? DISK_SURFACE_LOWER : DISK_SURFACE_UPPER);
    if (response != DIAGNOSTIC_RESPONSE_OK) {
        drawbridge_fdd_log("DrawBridge: Failed to select side %d: %d", head, response);
        if (response == DIAGNOSTIC_RESPONSE_MEDIA_TYPE_MISMATCH) {
            drawbridge_fdd_log(" (MEDIA_TYPE_MISMATCH - trying to re-detect disk)");
            /* Try to re-detect disk geometry */
            if (detect_floppy_geometry(dev)) {
                drawbridge_fdd_log("DrawBridge: Re-detected geometry: tracks=%d, heads=%d, sectors=%d, HD=%s\n",
                                  dev->tracks, dev->heads, dev->sectors, dev->is_hd ? "true" : "false");
                /* Retry with new geometry */
                response = arduino_interface_select_surface(dev->arduino, 
                    (head == 0) ? DISK_SURFACE_LOWER : DISK_SURFACE_UPPER);
                if (response != DIAGNOSTIC_RESPONSE_OK) {
                    drawbridge_fdd_log("DrawBridge: Still failed after re-detection: %d\n", response);
                    memset(buffer, 0, 512);
                    return;
                }
            } else {
                drawbridge_fdd_log("DrawBridge: Re-detection failed\n");
                memset(buffer, 0, 512);
                return;
            }
        } else {
    ...
    if (!dev->track_data_valid || dev->cached_track != track || dev->cached_side != head) {
        ...
        while (retry_count < max_retries) {
            ...
            if (retry_count < max_retries - 1) {
                ...
                uint32_t calibration_track = (drawbridge_cylinder < 40) ? drawbridge_cylinder + 30 : drawbridge_cylinder - 30;
                ...
                arduino_interface_select_track(dev->arduino, drawbridge_cylinder);  /* Seek back */
            }
            retry_count++;
        }
        
        if (response != DIAGNOSTIC_RESPONSE_OK) {
            drawbridge_fdd_log("DrawBridge: Failed to read track %d side %d: %d", track, head, response);
            if (response == DIAGNOSTIC_RESPONSE_MEDIA_TYPE_MISMATCH) {
                drawbridge_fdd_log(" (MEDIA_TYPE_MISMATCH - disk may be different density than detected)");
                /* Try reading as the opposite density */
                bool try_hd = !dev->is_hd;
                drawbridge_fdd_log("\nDrawBridge: Trying to read as %s disk instead...\n", try_hd ? "HD" : "DD");
                
                data_length = try_hd ? RAW_TRACKDATA_LENGTH_HD : RAW_TRACKDATA_LENGTH_DD;
                response = arduino_interface_read_current_track(dev->arduino, dev->track_data, data_length, true);
                if (response == DIAGNOSTIC_RESPONSE_OK) {
                    drawbridge_fdd_log("DrawBridge: Success! Disk is actually %s, updating geometry\n", try_hd ? "HD" : "DD");
                    dev->is_hd = try_hd;
                    if (try_hd) {
                        dev->sectors = 18;
                        dev->disk_flags = 0x08; /* Double sided */
                    ...
                    /* Update Arduino with correct capacity */
                    DiagnosticResponse set_response = arduino_interface_set_disk_capacity(dev->arduino, try_hd);
                    if (set_response != DIAGNOSTIC_RESPONSE_OK) {
                        drawbridge_fdd_log("DrawBridge: Warning: Failed to set corrected disk capacity: %d\n", set_response);
                    } else {
                        drawbridge_fdd_log("DrawBridge: Successfully set Arduino disk capacity to %s\n", try_hd ? "HD" : "DD");
                    }
                } else {
                    drawbridge_fdd_log("DrawBridge: Still failed with opposite density: %d\n", response);
                    memset(buffer, 0, 512);
                    return;
                }
            } else {
        ...
        /* Dump sample data for track 0 to understand format */
        if (track == 0) {
            char label[64];
            snprintf(label, sizeof(label), "Track %d Head %d (len=%d)", track, head, data_length);
            dump_track_data_sample(dev->track_data, data_length, label);
        }
    } else {
...
drawbridge_load(int drive, char *fn)
{
    ...
    /* Rewind to track 0 */
    response = arduino_interface_find_track0(dev->arduino);
    if (response != DIAGNOSTIC_RESPONSE_OK) {
        drawbridge_fdd_log("DrawBridge: Failed to find track 0: %d\n", response);
        /* Continue anyway, this might be recoverable */
    }
