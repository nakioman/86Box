...
typedef struct codeblock_t {
    ...
    uint16_t  incoming_link_head;   /* Head of incoming link list (blocks that jump to us) */
    uint16_t  incoming_link_next;   /* Next in incoming link list (per-block, for source blocks) */
#endif
} codeblock_t;
